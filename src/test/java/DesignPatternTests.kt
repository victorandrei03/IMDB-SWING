...
//    }
    @Test fun strategyTest() {
        val maxScore = 5.0f
        try {
            val strategy = Class.forName("org.example.strategy.ExperienceStrategy")
    ...
    @Test fun observerTest() {
        ...
        val subject: Class<*>
        try {
            observer = Class.forName("org.example.observer.Observer")
        } catch (e: ClassNotFoundException) {
        ...
        try {
            subject = Class.forName("org.example.observer.Subject")
        } catch (e: ClassNotFoundException) {
