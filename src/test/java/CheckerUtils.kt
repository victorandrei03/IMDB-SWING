    ...
    companion object {
        ...
        fun loadUsers() : MutableList<User<*>> {
            ...
            accounts.forEach(Consumer { account: JsonNode ->
                ...
                if (accountNode.has("favoriteProductions")) {
                    val x = IMDB.getInstance().productions
                            .filter { production: Production ->
                                accountNode["favoriteProductions"].toString().contains(production.title)
                            }
                            .toList()

                    favorites.addAll(x)
                    accountNode.remove("favoriteProductions")
                }
                if (accountNode.has("favoriteActors")) {
                    val x = IMDB.getInstance().actors
                            .filter { actor: Actor ->
                ...
                if (accountNode.has("productionsContribution")) {
                    contributions.addAll(
                        IMDB.getInstance().productions
                            .stream()
                            .filter { production: Production ->
                                accountNode["productionsContribution"].toString().contains(production.title)
                            }
                            .toList()
                    )
                    accountNode.remove("productionsContribution")
                }
